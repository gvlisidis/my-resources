...
@section('content')
    <div class="fixed inset-0 container p-4 m-auto bg-white rounded-md shadow-md max-h-175">
        <h3 class="mb-4 text-lg font-bold">Add New Snippet</h3>
        <form class="w-full" action="{{ route('snippets.store') }}" method="post">
            ...
            <div class="flex flex-col mb-4">
                ...
                <textarea name="body" id="body" class="w-full px-4 py-2 border-gray-500 rounded rounded-smt" rows="17" value=""></textarea>
                @error('body')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
