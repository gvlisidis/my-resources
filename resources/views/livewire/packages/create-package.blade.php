...
>
    <h3 class="mb-4 text-lg font-bold">{{ $method == 'create' ? 'Add New' : 'Edit'}} Package</h3>
    <form class="w-104" wire:submit.prevent="create" action="#" method="post">
        ...
        <div class="flex flex-col mb-4">
            <label for="owner" class="font-bold">Owner</label>
            <input type="text" name="owner" wire:model='owner' id="owner"
                class="w-full px-4 py-2 border-gray-500 rounded" value="" />
            @error('owner')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        ...
        <div class="flex mt-8 justify-between">
            ...
            <div>
                @if($method == 'update')
                    <x-button class="mr-4 bg-red-500 hover:bg-red-600" wire:click.prevent="delete({{ $package_id }})">
                        Delete
                    </x-button>
                @endif
            </div>
